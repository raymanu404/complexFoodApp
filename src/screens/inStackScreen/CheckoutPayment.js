...
function CheckoutPayment({ navigation, route }) {
  ...
  const { email, firstName, lastName, amount, city, country, address } = route.params
  ...
  const confirmPaymentFunction = async () => {
    try {
      ...
      if (userInfoBilling.completeCart && userInfoBilling.validDataCard) {
        ...
        if (error) {
          Alert.alert('Error', `error message: ${error.message} with error code: ${error.code}`)
        } else {
    ...
    <StripeProvider publishableKey={publishableKey} merchantIdentifier="merchant.identifier">
      ...
      >
        <View style={styles.userInputContainer}>
          ...
          />
          <TouchableOpacity activeOpacity={0.8} onPress={() => initializePaymentSheet()}>
            <UserField
              widthStyle={240}
