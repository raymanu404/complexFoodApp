  ...
  const addToCard = async (props) => {
    ...
    if (props.quantity > 0) {
      ...
      try {
        ...
        const response = await api_axios.post(`/shoppingItems/create/${resultBuyerId}`, dataToSend, headers)
  ...
  const renderFront = (props) => {
    return (
      <View style={[styles.image_container, { backgroundColor: colors.white }]}>
        ...
        <Text style={[styles.price_menu, { color: colors.backgroundButtonActive, marginTop: -10 }]}>
          {props.price} RON
        </Text>
    ...
    <View style={styles.container}>
      ...
      >
        {Array.from(data).length === 0 ? (
          ...
          <>
            <FlatList data={data} renderItem={renderMenuItem} keyExtractor={(item) => item.key} />
          </>
        )}
        <>
