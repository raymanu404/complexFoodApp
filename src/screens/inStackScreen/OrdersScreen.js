...
import { View, StyleSheet, Text, Dimensions, FlatList, TouchableOpacity, Modal, Switch } from 'react-native'
...
function OrdersScreen({ navigation, route }) {
  ...
  useEffect(() => {
    const getOrders = async () => {
      try {
        ...
        const sortOrderDesc = Array.from(response.data).sort(function (a, b) {
  ...
  const openCloseFiltersModalHandler = () => setIsModalVisible(() => !isModalVisible)
  ...
  const updateFilteredOrdersByTypeOfSorting = (typeOfSort, myFilteredList) => {
    switch (typeOfSort) {
      case 'Asc':
        const sortOrderAsc = Array.from(myFilteredList).sort(function (a, b) {
          return new Date(a.datePlaced) - new Date(b.datePlaced)
        })
        setFilterOrders(sortOrderAsc)
        break
      case 'Desc':
        const sortOrderDesc = Array.from(myFilteredList).sort(function (a, b) {
          return new Date(b.datePlaced) - new Date(a.datePlaced)
        })
        setFilterOrders(sortOrderDesc)
        break
      default:
        const sortOrderDesc1 = Array.from(myFilteredList).sort(function (a, b) {
          return new Date(b.datePlaced) - new Date(a.datePlaced)
        })
  ...
  const toggleSwitchHistory = () => {
    ...
    if (!isSwitchEnabled.history) {
      const filterOrdersByOrderStatusDone = Array.from(orders).filter((x) => x.status === 3)
      updateFilteredOrdersByTypeOfSorting(sorting.typeOfSorting, filterOrdersByOrderStatusDone)
    }
  }
  ...
  const toggleSwitchPlaced = () => {
    ...
    if (!isSwitchEnabled.placed) {
      const filterOrdersByOrderStatusPlaced = Array.from(orders).filter((x) => x.status === 1)
      updateFilteredOrdersByTypeOfSorting(sorting.typeOfSorting, filterOrdersByOrderStatusPlaced)
    }
  }
  ...
  const toggleSwitchDiscount = () => {
    ...
    if (!isSwitchEnabled.withDiscount) {
      const filterOrdersByDiscount = Array.from(orders).filter((x) => x.discount !== 0)
      updateFilteredOrdersByTypeOfSorting(sorting.typeOfSorting, filterOrdersByDiscount)
    }
  }
  ...
  const toggleCheckBox10Discount = () => {
    ...
    if (!isSwitchEnabled.withDiscount10) {
      const filterOrdersBy10Discount = Array.from(orders).filter((x) => x.discount === 10)
      updateFilteredOrdersByTypeOfSorting(sorting.typeOfSorting, filterOrdersBy10Discount)
    }
  }
  ...
  const toggleCheckBox20Discount = () => {
    ...
    if (!isSwitchEnabled.withDiscount20) {
      const filterOrdersBy20Discount = Array.from(orders).filter((x) => x.discount === 20)

      updateFilteredOrdersByTypeOfSorting(sorting.typeOfSorting, filterOrdersBy20Discount)
    }
  }
  ...
  const toggleCheckBox30Discount = () => {
    ...
    if (!isSwitchEnabled.withDiscount30) {
      const filterOrdersBy30Discount = Array.from(orders).filter((x) => x.discount === 30)

      updateFilteredOrdersByTypeOfSorting(sorting.typeOfSorting, filterOrdersBy30Discount)
    }
  }
  ...
  return (
    <View style={styles.orderContainer}>
      <View style={styles.backButton}>
        <Icon name="arrowleft" type="ant-design" style={styles.icon} size={26} onPress={() => navigation.goBack()} />
      </View>

      {/* --------------------------- FILTERS CONTAINER -------------------- */}
      <View style={styles.filterContainer}>
        <TouchableOpacity style={styles.filterItem} onPress={() => openCloseFiltersModalHandler()} activeOpacity={0.5}>
          <Icon
          ...
          <View style={styles.modalView}>
            ...
            <View style={styles.filterContainer}>
              <TouchableOpacity
                ...
                activeOpacity={0.5}
              >
                <Icon name={'closecircleo'} type={'antdesign'} color={colors.white} size={30} />
              </TouchableOpacity>
            </View>
            ...
            <View style={styles.modalInsideContainer}>
              ...
              <View style={styles.flexDirectionRow}>
                <Text style={styles.textModalFilterItem}>Comenzile plasate</Text>
                <Switch
                  trackColor={{ false: '#767577', true: '#81b0ff' }}
                  thumbColor={'#f4f3f4'}
              ...
              <View style={styles.flexDirectionRow}>
                <Text style={styles.textModalFilterItem}>Comenzile cu reducere</Text>
                <Switch
                  trackColor={{ false: '#767577', true: '#81b0ff' }}
                  thumbColor={'#f4f3f4'}
              ...
              <View style={styles.flexDirectionRow}>
                <Text style={styles.textModalFilterItem}>Comenzi cu 10 % Reducere</Text>
                <Switch
                  trackColor={{ false: '#767577', true: '#81b0ff' }}
                  thumbColor={'#f4f3f4'}
              ...
              <View style={styles.flexDirectionRow}>
                <Text style={styles.textModalFilterItem}>Comenzi cu 20 % Reducere</Text>
                <Switch
                  trackColor={{ false: '#767577', true: '#81b0ff' }}
                  thumbColor={'#f4f3f4'}
              ...
              <View style={styles.flexDirectionRow}>
                <Text style={styles.textModalFilterItem}>Comenzi cu 30 % Reducere</Text>
                <Switch
                  trackColor={{ false: '#767577', true: '#81b0ff' }}
                  thumbColor={'#f4f3f4'}
              ...
              <View style={styles.modalInsideContainer}>
                <View style={styles.flexDirectionRow}>
                  <RadioButton.Group onValueChange={(newValue) => sortHandler(newValue)} value={sorting.typeOfSorting}>
                    {/* <View style={styles.genderContainer}> */}
                    <View style={styles.genderItem}>
                      <Text style={styles.genderTextItem}>Sorteaza dupa cele mai recente</Text>
                      <RadioButton value="Desc" color={'#f4f3f4'} />
                    </View>
                    <View style={styles.genderItem}>
                      <Text style={styles.genderTextItem}>Sorteaza dupa cele mai vechi</Text>
      ...
      {filterOrders.length !== 0 ? (
        <View style={styles.ordersFlatListContainer}>
          <FlatList data={filterOrders} keyExtractor={(item) => item.id} renderItem={renderOrder} />
        </View>
      ) : (
