...
import { useConfirmPayment, StripeProvider, CardField, CardForm, useStripe } from '@stripe/stripe-react-native'
...
function PayDesk({ navigation, route }) {
  ...
  const { buyerId, email, firstName, lastName, phoneNumber } = route.params.userInfo
  ...
  useEffect(() => {
    const getPublishableKey = async () => {
      try {
        ...
        const response = await api_axios.get('/payment/publishableKey', headers)
  ...
  const fetchPayementIntentClientSecret = async () => {
    try {
      ...
      const reponseCreatePaymentIntent = await api_axios.post('/payment/create-payment-intent', dataToSend, headers)

      if (reponseCreatePaymentIntent.status === 200) {
        const clientSecretFromApi = reponseCreatePaymentIntent.data.clientSecret
    ...
    <StripeProvider publishableKey={publishableKey} merchantIdentifier="merchant.identifier">
      ...
      >
        <View style={styles.userInputContainer}>
          ...
          />
          {!loading ? (
            <TouchableOpacity activeOpacity={0.8} style={{ marginTop: 10 }} onPress={() => confirmPaymentFunction()}>
              <UserField
                widthStyle={240}
            ...
            <ActivityIndicator size="large" color={colors.backgroundButtonActive} style={{ marginTop: 10 }} />
          )}
          <>
