  ...
  const changePasswordHandler = async () => {
    ...
    if (conds) {
      ...
      try {
        ...
        const responseChangePassword = await api_axios.patch(`/buyers/change-password/${idUser}`, dataToSend, headers)

        if (responseChangePassword.status === 200) {
          if (String(responseChangePassword.data).startsWith('Password was updated successfully!')) {
            setShowRenderToast({
              ...showRenderToast,
              success: true,
              fail: false,
    ...
    >
      <View style={styles.container}>
        ...
        <View style={styles.displayColumn}>
          ...
          <Text style={styles.textErrorInput}>{invalidInput.passwordError ? invalidInput.passwordTypeError : ''}</Text>
        </View>

        {/* ------------------------------------- CONFIRMARE PAROLA -------------------------------------- */}
        <View style={styles.displayColumn}>
          <View style={onFocusInput.re_passwordOnFocus ? styles.textInputContainerOnFocus : styles.textInputContainer}>
            <Icon
              name={'unlock'}
              type="feather"
              color={colors.backgroundButtonActive}
