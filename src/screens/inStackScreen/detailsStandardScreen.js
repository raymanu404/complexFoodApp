  ...
  const addToCart = async () => {
    try {
      if (quantity !== 0) {
        ...
        const response = await api_axios.post(`/shoppingItems/create/${menuDataInCart.userId}`, dataToSend, headers)
  ...
  return (
    <View style={styles.container}>
      ...
      <ScrollView style={{ flex: 1 }}>
        <Animatable.View style={styles.image_container} animation={'bounceIn'} duration={800}>
          <Image source={{ uri: menuDataInCart.src }} style={styles.image} resizeMode="cover" />
        </Animatable.View>
        <View style={[styles.image_container, { width: menu_container_width, marginBottom: 80 }]}>
          ...
          <View style={styles.buttonsAddRemoveQuantity}>
            ...
            <View style={styles.quantityLabel}>
              <Text style={[styles.title_menu, { color: colors.backgroundButtonActive, fontSize: 28 }]}>
                {`${quantity}`}{' '}
              </Text>
            </View>
            <TouchableOpacity activeOpacity={0.8}>
