  ...
  const confirmHandler = async () => {
    ...
    if (letters.length === 6) {
      ...
      try {
        ...
        const response = await api_axios.patch(`/buyers/confirm/${idUser}`, code, headers)
        ...
        if (response.data === 'Buyer-ul a fost confirmat cu succes!') {
          ...
          setTimeout(() => {
            if (forgotPassword) {
              navigation.navigate('ForgotPasswordScreen', { idUser })
            } else {
              navigation.goBack()
            }
          }, 1000)
      ...
      <View style={styles.container}>
        <View style={styles.inputContainer}>
          ...
          <View style={styles.button}>
            <TouchableOpacity activeOpacity={0.95} onPress={() => confirmHandler()}>
