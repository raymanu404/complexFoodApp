  ...
  const saveSettingsHandler = async () => {
    try {
      ...
      if (settingsMode.showSettingsMode) {
        ...
        const response = await api_axios.put(`/buyers/update/${buyerID}`, userDataUpdate, headers)
      ...
      <View style={styles.container}>
        <View style={styles.avatarContainer}>
          <Avatar
            size={120}
            rounded
            title={String(dataFromAPI.firstName).charAt(0) + '' + String(dataFromAPI.lastName).charAt(0)}
        ...
        <View style={styles.header}>
          <>
            ...
            <View style={styles.editProfile}>
              <TouchableOpacity onPress={() => settingsModeHandler()} activeOpacity={0.8}>
                <UserField
                  widthStyle={150}
            ...
            <View style={styles.headerRight}>
              <View style={styles.centerRow}>
                <Text style={styles.text_balance}>
                  SOLD: {settingsMode.showBalance ? `${Number(dataFromAPI.balance).toFixed(2)}` : '****'} RON
                </Text>
                <TouchableOpacity onPress={() => showHideBalanceHandler()} activeOpacity={0.5}>
                  <Icon
              ...
              <View style={styles.logoutButton}>
                <TouchableOpacity onPress={() => logoutHandler()} activeOpacity={0.8}>
                  <UserField
                    widthStyle={130}
                    colorBackground={colors.blackGrey}
            ...
            <ScrollView style={styles.userProfile}>
                  ...
                  {!settingsMode.showSettingsMode ? (
                    <>
                      <UserField
                        ...
                        typeIcon={'antdesign'}
                        labelField={'Nume'}
                        dataField={dataFromAPI.firstName + ' ' + dataFromAPI.lastName}
                      />
                      <UserField
                        widthStyle={width - 30}
                        colorBackground={colors.blackGrey}
              ...
              <View style={styles.userDataInfo}>
                ...
                <TouchableOpacity onPress={() => changePasswordHandler()} activeOpacity={0.8}>
                  <UserField
                    widthStyle={width - 30}
                    colorBackground={colors.blackGrey}
                ...
                <>
                  {settingsMode.showChangePassword ? (
                    <>
                      ...
                      <Text style={styles.textError}>{textError.passwordTextError}</Text>
                    </>
                  ) : null}
                </>
                <TouchableOpacity activeOpacity={0.8} onPress={() => depunereHandler()}>
                  <UserField
                    widthStyle={width - 30}
                    colorBackground={colors.blackGrey}
                    nameIcon={'creditcard'}
                ...
                <TouchableOpacity activeOpacity={0.8} onPress={() => showHideTicketsHandler()}>
                  <UserField
                    widthStyle={width - 30}
                    colorBackground={colors.blackGrey}
                ...
                {settingsMode.showSaveButton ? (
                  <>
                    <View style={styles.settingsModeContainer}>
                      <TouchableOpacity onPress={() => saveSettingsHandler()} activeOpacity={0.8}>
                        <UserField
                          widthStyle={150}
                          colorBackground={colors.blackGrey}
