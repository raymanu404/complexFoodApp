...
const defaultMenusData = [
  {
    id: '3',
    image: 'https://blobcontainercomplexfood.blob.core.windows.net/complexfoodcontainer1/15.jpg',
    title: 'Cheese Cake',
    price: 5.99,
    quantity: 0,
...
function Home({ navigation }) {
  ...
  const MenuItem = (props) => (
    <View style={styles.menuContainer}>
      <TouchableHighlight underlayColor={colors.white} onPress={() => goToDetailsStandard(props)}>
        <Image source={{ uri: props.image }} style={styles.menu_standard_image} resizeMode="cover" />
      </TouchableHighlight>
  ...
  const SearchMenuItem = (props) => (
    <View style={styles.searchMenuContainer}>
      <TouchableHighlight underlayColor={colors.white} onPress={() => goToDetailsStandard(props)}>
        <Image source={{ uri: props.image }} style={styles.search_menu_image} resizeMode="cover" />
      </TouchableHighlight>
      <Text style={styles.title_menu}>{props.title} </Text>
      <Text style={styles.price_menu}>{props.price} RON</Text>
    </View>
  ...
  const Categories = (props) => (
    <View style={styles.categories_container}>
      <TouchableOpacity onPress={() => goToDetailsCategories(props)} style={{ marginTop: 3 }}>
        <Image source={props.src} style={styles.category_image} />
      </TouchableOpacity>
      <Text style={[styles.title_menu, { fontSize: 12, fontWeight: '500' }]}>{props.title}</Text>
    </View>
  )
  ...
  const goToDetailsCategories = (props) => {
    const menuDataForCategories =
      menuData.length !== 0
        ? menuData.filter((item) => item.category === props.type && item.isInStock === true)
        : defaultMenusData.filter((item) => item.category === props.type)

    const dataCategories = menuDataForCategories.map(function (row) {
      return {
        title: row.title,
        src: row.image,
        price: row.price,
  ...
  const onSearchBarButton = () => {
    if (String(infoToSearch) !== '') {
      ...
      const searchMenuStartWithOrIncludes = Array.from(menuData).filter(
        (x) =>
  ...
  const openCloseSearchMenuModalHandler = () => setIsModalVisible(() => !isModalVisible)
      ...
      <View style={styles.container}>
        ...
        <View style={styles.header}>
          ...
          <View style={styles.searchBar}>
            ...
            />
            <TouchableOpacity style={styles.icon} onPress={() => onSearchBarButton()}>
              <Icon name={'search'} type="feather" color={colors.backgroundButtonActive} />
            </TouchableOpacity>
          </View>
                ...
                <View style={styles.modalView}>
                  ...
                  <View style={styles.flexDirectionRow}>
                    ...
                    >
                      <TextInput
                        ...
                        placeholderTextColor={colors.backgroundButtonActive}
                      />
                      <TouchableOpacity style={styles.icon} onPress={() => onSearchBarModalButton()}>
                        <Icon name={'search'} type="feather" color={colors.backgroundButtonActive} />
                      </TouchableOpacity>
                    </View>
                  </View>
                  <View style={styles.modalInsideContainer}>
                    {searchMenuData.length !== 0 ? (
                      ...
                      >
                        <RenderEmptyList title_message={'Meniul acesta nu exista!'} />
                      </View>
                    )}
                  </View>
                </View>
        ...
        <View style={styles.footer}>
          ...
          <View style={styles.menuStandardContainer}>
            ...
            <FlatList
              ...
              data={
                menuData.length !== 0
                  ? menuData.filter((el) => Number(el.category) === enum_categories.STANDARD && el.isInStock === true)
                  : defaultMenusData.filter((el) => Number(el.category) === enum_categories.STANDARD)
              }
              keyExtractor={(item) => item.id}
          ...
          <View style={styles.categories}>
            <Text style={[styles.textSuggest, { color: colors.blackGrey, right: width - width * 0.6 }]}>Categorii</Text>
            <FlatList
              horizontal={true}
              data={categoriesData}
          ...
          <View style={styles.ordersContainer}>
            <Text style={[styles.textSuggest, { color: colors.blackGrey, marginLeft: 10 }]}>Comenziile mele</Text>
            <Animatable.View animation={'pulse'} duration={800} style={styles.orderBackground}>
              <Icon
