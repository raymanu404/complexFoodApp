...
import { View, StyleSheet, Text, Dimensions, Image, TouchableWithoutFeedback, Keyboard, FlatList } from 'react-native'
...
function Cart({ navigation }) {
    ...
    React.useCallback(() => {
      const getDataFromCart = async () => {
        try {
          ...
          const response = await api_axios.get(`/shoppingItems/get_items/${buyerID}`, headers)
  ...
  const removeFromQuantity = (props) => {
    ...
    if (props.quantity > 0) {
      ...
      const newTotalPrice =
        totalPrice - Number(newDataMenu[props.myKey].quantity) * Number(newDataMenu[props.myKey].price)
  ...
  const removeItemFromCart = async (props) => {
    ...
    try {
      ...
      const reponseDeleteItem = await api_axios.post(`/shoppingItems/create/${buyerID}`, itemToDelete, headers)
      if (reponseDeleteItem.status === 200 && reponseDeleteItem.data === 'Item was deleted successesfully!') {
  ...
  const renderFront = (props) => {
    return (
      <View style={[styles.image_container, { backgroundColor: colors.white }]}>
        ...
        <Text style={[styles.title_menu, { color: colors.backgroundButtonActive }]}>{props.title}</Text>
        <View
          style={{
            flexDirection: 'row',
  ...
  const renderBack = (props) => {
    return (
      <View style={[styles.image_container, { width: menu_container_width }]}>
        ...
        <Text style={[styles.details_text, { fontWeight: '700', marginBottom: -10 }]}>Detalii Produs</Text>
  ...
  const setCodeCouponHandler = (code, typeOfDiscount) => {
    ...
    newTotalPriceWithDiscount = totalPrice - (originalTotalPrice * discount) / 100 - PRICE_OF_COUPON
  ...
  const confirmCommandHandler = async () => {
    try {
      ...
      if (checkoutBalanceRedirectOnPaymentScreen(userDataLogin.balance)) {
        ...
        const response = await api_axios.patch(`/carts/confirm/${buyerID}`, couponCodeFromBuyer, headers)
    ...
    <View style={styles.container}>
      {menuDataInCart.length !== 0 ? (
        <View style={styles.menu_container}>
          <FlatList renderItem={renderMenuItem} keyExtractor={(item) => item.id} data={menuDataInCart} />
          <View style={styles.payment_container}>
            <View style={styles.payment_view}>
              <View style={styles.couponContainer}>
                <TouchableOpacity activeOpacity={0.8}>
                  <View style={styles.button}>
                    <Text style={styles.buttonText} onPress={applyUserCouponHandler}>
                      Aplica Cupon
                    </Text>
                  </View>
                </TouchableOpacity>
                <Text style={styles.textCouponCode}>
                  {couponCode.typeOfCoupon === 0 ? '' : String(`Discount: ${couponCode.typeOfCoupon}0% reducere`)}
                </Text>
              </View>

              <View style={[styles.button, { backgroundColor: colors.white, width: 160, marginBottom: 30 }]}>
                <Text style={[styles.buttonText, { color: colors.black, fontSize: 16 }]}>Total: {totalPrice} RON</Text>
              </View>
            </View>
            ...
            <TouchableOpacity activeOpacity={0.8}>
              <View style={styles.button}>
                <Text style={styles.buttonText} onPress={() => confirmCommandHandler()}>
                  Plateste
                </Text>
              </View>
            </TouchableOpacity>
        ...
        <ConfirmedOrder title_message="Multumim pentru comanda!" orderCode={confirmCart.orderCode} />
      ) : (
