        ...
        >
          <View style={styles.toastRow}>
            <Icon
              ...
              type={'antdesign'}
              size={24}
              color={props.status === 'success' ? colors.successToastColor : colors.failToastColor}
              style={{ paddingLeft: 5 }}
            />

            <View style={styles.toastText}>
              <Text style={{ fontWeight: 'bold', fontSize: 15 }}>{props.title_message}</Text>
              <Text style={{ fontSize: 14 }}>{props.message}</Text>
            </View>
            <TouchableOpacity onPress={instantPopOut}>
              <Icon name={'x-circle'} type={'feather'} size={24} color={colors.blackGrey} style={{ paddingLeft: 5 }} />
            </TouchableOpacity>
          </View>
        </Animated.View>
      ) : (
